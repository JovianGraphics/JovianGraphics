...
#define GANYMEDE_ENUM_FLAGS_SEQ_X(s, data, elem) BOOST_PP_TUPLE_ELEM(0, elem) = BOOST_PP_TUPLE_ELEM(1, elem)
#define GANYMEDE_ENUM_FLAGS_SEQ_ELEM(s, data, elem) BOOST_PP_TUPLE_ELEM(0, elem)
...
#define GANYMEDE_ENUM_FLAGS(name, val_seq)                                                     \
    enum name : uint32 {                                                                       \
        BOOST_PP_SEQ_ENUM(BOOST_PP_SEQ_TRANSFORM(GANYMEDE_ENUM_FLAGS_SEQ_X, ~, val_seq))       \
    };                                                                                         \
    static const char* BOOST_PP_CAT(name, Strings[] = ) {                                      \
        BOOST_PP_SEQ_ENUM(BOOST_PP_SEQ_TRANSFORM(GANYMEDE_DEFINE_ENUM_VAL_STR, name, val_seq)) \
    };                                                                                         \
    static std::string name##ToString(name value) {                                            \
        std::stringstream ss;                                                                  \
        BOOST_PP_SEQ_FOR_EACH(GANYMEDE_ENUM_FLAGS_PRINT_NAME, name, BOOST_PP_SEQ_TRANSFORM(GANYMEDE_ENUM_FLAGS_SEQ_ELEM, ~, val_seq)) \
        return ss.str();                                                                       \
    };